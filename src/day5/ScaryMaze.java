...
int background = maze.getRGB(1, 1);
int bc = -8102500;
int wc = -1317100;
		//6. if the mouse falls off the path (if it is on the background)
		if (mouseColor == background) {
			scare();
			showScaryImage("shrek.jpg");
			
		
		}
		else{
			if (mouseColor == wc) {
				JOptionPane.showMessageDialog(null,"You win....Barely.");
			}
	...
	private void showScaryImage(String shrek) {
		try {
			maze = ImageIO.read(getClass().getResource(shrek));
		} catch (Exception e) {
			System.err.println("Couldn't find this image: " + shrek);
		}
		repaint();
	}
